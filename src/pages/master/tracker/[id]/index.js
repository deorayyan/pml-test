...
import {
  clearMasterTrackerDetail,
  getMasterTracker,
} from "@/redux/slices/masterTrackerSlice";
...
export default function MasterTrackerEditPage() {
  ...
  const getDetail = async () => {
    if (router?.query?.id) {
      const { payload: data } = await dispatch(
        getMasterTracker(router.query.id)
      );
      setInitialValue({
        ...data,
      });
  ...
  return <MasterTrackerForm readOnly initialValue={initialValue} />;
}
